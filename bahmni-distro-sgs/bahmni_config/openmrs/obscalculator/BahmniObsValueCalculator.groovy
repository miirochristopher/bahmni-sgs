...
public class BahmniObsValueCalculator implements ObsValueCalculator {
	...
	// CDC tool calculator
        BahmniObservation physicalHealthObs = find("cdc_physical_health", observations, null)
        BahmniObservation mentalHealthObs = find("cdc_mental_health", observations, null)

        if(hasValue(physicalHealthObs) && hasValue(mentalHealthObs)){
            def cdcTotalScore = "cdc_totalScore"
            BahmniObservation cdcTotalScoreObs = find(cdcTotalScore, observations, null)
            parent = obsParent(physicalHealthObs, null)
            Date obsDatetime = getDate(physicalHealthObs)

            def physicalHealthObsValue = physicalHealthObs.getValue()
            def mentalHealthObsValue = mentalHealthObs.getValue()

            def total = physicalHealthObsValue + mentalHealthObsValue
            if(cdcTotalScoreObs == null)
                cdcTotalScoreObs = createObs(cdcTotalScore, parent, bahmniEncounterTransaction, obsDatetime) as BahmniObservation
            cdcTotalScoreObs.setValue(total)
	...
	// CDC tool proms calculator
        BahmniObservation physicalHealthPromsObs = find("cdc_physical_health_proms", observations, null)
        BahmniObservation mentalHealthPromsObs = find("cdc_mental_health_proms", observations, null)

        if(hasValue(physicalHealthPromsObs) && hasValue(mentalHealthPromsObs)){
            def cdcTotalScore = "cdc_totalScore_proms"
            BahmniObservation cdcTotalScoreObs = find(cdcTotalScore, observations, null)
            parent = obsParent(physicalHealthPromsObs, null)
            Date obsDatetime = getDate(physicalHealthPromsObs)

            def physicalHealthObsValue = physicalHealthPromsObs.getValue()
            def mentalHealthObsValue = mentalHealthPromsObs.getValue()
	...
	// Vas Pain Total Score
        BahmniObservation vasPainObs = find("VAS Pain", observations, null)
        if(hasValue(vasPainObs)){
            def vasPainTotalScore = "Vas Pain Total Score"
            BahmniObservation vasPainTotalScoreObs = find(vasPainTotalScore, observations, null)
            parent = obsParent(vasPainObs, null)
            Date obsDatetime = getDate(vasPainObs)

            def vasPainObsValue = vasPainObs.getValue().value
            def total = vasPainData(vasPainObsValue)
            if(vasPainTotalScoreObs == null)
                vasPainTotalScoreObs = createObs(vasPainTotalScore, parent, bahmniEncounterTransaction, obsDatetime) as BahmniObservation
            vasPainTotalScoreObs.setValue(total)
    ...
    private static Integer vasPainData(String value){
        switch(value){
            case "0":
                return 0
            case "1":
